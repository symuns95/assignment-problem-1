...
const SingleTabComponent = ({ index, value, adminLists, userLists }) => {
  const userInfo = [{ name: "Fistname Lastname" }, { name: "District" }, { name: "Division" }, { name: "Active/Inactive" }, { btnTable: "Details View" }];
        ...
        <div className="single-tab-main">
          ...
          {adminLists &&
            adminLists.map((item, i) => {
              return <ProfileInfo key={i} userInfo={userInfo} bg={i % 2 === 0 ? true : false} />;
            })}
          {userLists &&
