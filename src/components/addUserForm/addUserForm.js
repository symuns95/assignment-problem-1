...
const MyTextInput = forwardRef(({ label, ...props }, ref) => {
  ...
  return (
    <div className={`input-field-main ${meta.error && "items-start active-error-input-margin"}`}>
      <label htmlFor={props.id || props.name}>{label}</label>
      <div className="input-field">
        <input ref={ref} className="text-input" {...field} {...props} />
        {meta.touched && meta.error ? <div className="error-inputField">{meta.error}</div> : null}
      </div>
    </div>
  );
});
...
const AddUserForm = () => {
  ...
  const newDivisionArray = [{ name: "", empty: "Select your division", value: "" }, ...divitionOfState];
    ...
    <div className="form-main">
      <Formik
        ...
        validationSchema={Yup.object({
          ...
          user_type: Yup.string().oneOf(["admin", "employee"], "Invalid Job Type").required("Required"),
      ...
      >
        <Form className="form-add-user">
          <MyTextInput label="First Name" name="first_name" type="text" placeholder="Enter your first name" ref={firstNameRef} />
          <MyTextInput label="Last Name" name="last_name" type="text" placeholder="Enter your last name" ref={firstNameRef} />
